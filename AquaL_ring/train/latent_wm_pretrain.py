...
def main(args):
    ...
    def generate_circular_mask(height, width, inner_radius, outer_radius):
        """生成一个圆环形的掩模（mask），inner_radius 表示内环半径，outer_radius 表示外环半径"""
        # 创建网格
    ...
    def gen_combined_latents(latents, wm_latent, scale=1.0, inner_radius=0.3, outer_radius=0.7):
        """生成水印并将其嵌入潜在空间。水印呈圆环形"""
        ...
        if cornerfy_aug:
            # 生成圆环形的水印掩模
            circular_mask = generate_circular_mask(height, width, inner_radius, outer_radius)
            wm_template = wm_latent * circular_mask.unsqueeze(0).unsqueeze(0)  # 扩展维度匹配 wm_latent
        else:
